//! Official BIP-39 English Word List (2048 words) sorted alphabetically for binary search.

pub static BIP39_WORDS: &[&'static str] = &[
    ...
    "patrol", "pattern", "pause", "pave", "payment", "peace", "peach", "peacock", "peak", "peanut",
    ...
    "pig", "pigeon", "pill", "pilot", "pin", "pine", "pipe", "pistol", "pitch", "pizza",
...
/// Checks if a lowercase word is a valid BIP-39 dictionary word in O(log N) time
pub fn is_bip39_word(word: &str) -> bool {
    let lower = word.to_ascii_lowercase();
    BIP39_WORDS.binary_search(&lower.as_str()).is_ok()
}

/// Validates whether a candidate list of words forms a legitimate BIP-39 mnemonic phrase.
/// Valid standard mnemonic lengths are 12, 15, 18, 21, and 24 words.
/// All words must exist in the BIP-39 wordlist.
pub fn validate_bip39_phrase(words: &[&str]) -> Option<String> {
    let valid_lengths = [12, 15, 18, 21, 24];
    for &target_len in &valid_lengths {
        if words.len() >= target_len {
            let candidate_slice = &words[..target_len];
            let all_valid = candidate_slice.iter().all(|w| is_bip39_word(w));
            if all_valid {
                return Some(candidate_slice.iter().map(|w| w.to_lowercase()).collect::<Vec<_>>().join(" "));
            }
        }
    }
    None
